...
/***********************************************/
char const* strLicense = "THIS SOFTWARE IS PROVIDED \"AS-IS\". THERE IS NO WARRANTY OF ANY KIND. "
...
// [[Rcpp::depends(BH)]]
// [[Rcpp::export]]
int run_simpers(std::string input_domain_complex_file_name,
                std::string input_range_complex_file_folder,
                std::string input_simplicial_map_file,
                std::string output_range_complex_with_annotation_file_name,
                std::string output_persistence_file_name,
                ...
                int max_dimension_)
{
  ...
  fThreshold = fThreshold_;
  max_dimension = max_dimension_;
//   // Convert to old C argc, argv equivalents
//   int argc = args.size();
//   std::vector< std::string > args_str = as< std::vector< std::string > >(args);
//
//   std::for_each(args_str.begin(), args_str.end(), [](const std::string s){
//     Rprintf("arg: (%s)\n", s.c_str());
//   });
//
//   std::vector< char* > cstrings;
//   cstrings.reserve(argc);
//   for(const std::string& s: args_str){ cstrings.push_back( (char*)(s.data()) ); }
//   cstrings.push_back(nullptr); // to be safe
//   char** argv = cstrings.data();
//
//
//   // char** argv = new char*[args_str.size()+2];
//   // const char* pn = "Simpers";
//   // argv[0] = (char*)(pn);
//   // for (std::size_t j = 0; j < args_str.size()+1; ++j){ // copy args
//   //   char* c_char = (char*)(args_str[j].c_str());
//   //   argv[j+1] = c_char;
//   // }
//   // argv[args_str.size()+1] = NULL;
//
//   Rcout << argv << std::endl;
// 	start = std::clock();
// 	std::string input_domain_complex_file_name;
...
// 	bool is_generator;
	...
	// 	max_dimension))
	// {
	  input_domain_complex_file_name = trim_copy(input_domain_complex_file_name);
	  Rcout << "Elementary mode: " << is_elementary << std::endl;
	  Rcout << "Input complex file: " << input_domain_complex_file_name << std::endl;
	  Rcout << "Input simplicial map file: " << trim_copy(input_simplicial_map_file) << std::endl;
	  Rcout << "max dimension: " << max_dimension << std::endl;
	  Rcout << "generators: " << is_generator << std::endl;
	  Rcout << "Output persistence file: " << output_persistence_file_name << std::endl;
	  input_simplicial_map_file = trim_copy(input_simplicial_map_file);
	  output_persistence_file_name = trim_copy(output_persistence_file_name);
		if (!is_elementary) {
			...
			if (is_generator)
				bTimeStamp = true;
			ifstream ifs_m;
			ifs_m.open(trim_copy(input_simplicial_map_file));
			if (!ifs_m.is_open()) {
				Rcout << "Can't open simplicial map file!" << endl;
				return 0;
			}
			filtration_step = 0;
			while (!(ifs_m.eof()))
		...
		{
			ofstream ofs_gen(generator_file);

			// 	std::unordered_map<int, std::vector<int>>
			std::vector<int> path;
			for (auto& time_gen: domain_complex.gen1){
			  ofs_gen << "Generator for ID " << time_gen.first << ": ";
        path = time_gen.second;
        std::for_each(path.begin(), path.end(), [&ofs_gen](const int id){
          ofs_gen << id << " ";
        });
        ofs_gen << std::endl;
			}
			ofs_gen.close(); ofs_gen.clear();
		}
		domain_complex.clearData();
		domain_complex.clearMemory();
		// domain_complex.~SimplicialTree();
...
// 		}
  ...
  std::vector<int>().swap(complexSizes);
  std::vector<int>().swap(accumulativeSizes);
	fThreshold = filtration_step = time_in_each_filtration_step = 0;
	vector<float>().swap(vecFiltrationScale);
	domain_complex.clearMemory();
	domain_complex.clearData();
	domain_complex.gen1.clear();
	dFuncTimeSum = dCollapseTime = dInsertTime = 0;
	max_dimension = collapseCount = smallCount = 0;
	persistences.clear();
	return 0;
}
